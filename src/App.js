...
import { Plus, Package, IndianRupee, Edit2, Trash2, Search, Scale, ShoppingCart, Receipt } from 'lucide-react';

const ProvisionStore = () => {
  const [products, setProducts] = useState([
    { id: 1, name: 'Rice (Basmati)', pricePerUnit: 120, weight: 1, weightUnit: 'kg', category: 'Grains' },
    { id: 2, name: 'Wheat Flour', pricePerUnit: 45, weight: 1, weightUnit: 'kg', category: 'Grains' },
    { id: 3, name: 'Sugar', pricePerUnit: 55, weight: 1, weightUnit: 'kg', category: 'Sweeteners' },
    { id: 4, name: 'Tea Powder', pricePerUnit: 500, weight: 500, weightUnit: 'gm', category: 'Beverages' }
  ]);
  ...
  const categories = ['Grains', 'Pulses', 'Spices', 'Oil & Ghee', 'Beverages', 'Sweeteners', 'Dairy', 'Snacks', 'Others'];
  ...
  const handleSubmit = () => {
    ...
    setFormData({ name: '', pricePerUnit: '', weight: '', weightUnit: 'kg', category: '' });
  ...
  const updateCartItemPrice = (id, pricePerUnit) => {
    setCart(prev => prev.map(item => 
      item.id === id ? { ...item, pricePerUnit: parseFloat(pricePerUnit) || 0 } : item
    ));
  };
  ...
  const cartTotal = cart.reduce((sum, item) => {
    const itemPrice = calculatePrice(item.pricePerUnit, item.weight, item.weightUnit);
      ...
      <div style={styles.maxWidth}>
        ...
        <div style={styles.statsGrid}>
          ...
          <div style={styles.statCard}>
            ...
            <div style={{...styles.iconBox, background: 'linear-gradient(135deg, #ef4444, #dc2626)'}}>
        ...
        {showBilling && (
          <div style={styles.glassCard}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px'}}>
              ...
              <button
                onClick={clearCart}
                style={{...styles.button, background: 'linear-gradient(135deg, #ef4444, #dc2626)'}}
              >
                Clear Cart
              </button>
            </div>
            {cart.length === 0 ? (
              <div style={styles.emptyState}>
                ...
                <h3 style={{fontSize: '1.5rem', margin: '20px 0 10px'}}>Cart is empty</h3>
              ...
              <>
                  ...
                  <div key={item.id} style={styles.cartItem}>
                    <div style={styles.cartItemDetails}>
                      ...
                      <div style={styles.cartEditRow}>
                        <span style={{color: 'rgba(255, 255, 255, 0.7)', minWidth: '60px'}}>Weight:</span>
                        <input
                          type="number"
                          value={item.weight}
                          ...
                          min="0"
                        />
                        <span style={{color: 'rgba(255, 255, 255, 0.7)'}}>{item.weightUnit}</span>
                      </div>
                      <div style={styles.cartEditRow}>
                        <span style={{color: 'rgba(255, 255, 255, 0.7)', minWidth: '60px'}}>Price/Unit:</span>
                      ...
                      <div style={{color: 'rgba(255, 255, 255, 0.8)', fontSize: '0.9rem'}}>
                        Unit Total: ₹{calculatePrice(item.pricePerUnit, item.weight, item.weightUnit).toFixed(2)}
                      </div>
                    </div>
                    <div style={styles.cartItemControls}>
                      ...
                      <div style={{fontWeight: 'bold', fontSize: '1.2rem', marginTop: '10px'}}>
                        ₹{(calculatePrice(item.pricePerUnit, item.weight, item.weightUnit) * item.quantity).toFixed(2)}
                      </div>
                    </div>
                  </div>
                ))}
                <div style={{
                  ...
                  <h3 style={{color: 'white', fontSize: '1.5rem', margin: '0'}}>Total Amount</h3>
                  <h3 style={{color: 'white', fontSize: '2rem', margin: '0'}}>₹{cartTotal.toFixed(2)}</h3>
                </div>
                <button
                  style={{...styles.button, width: '100%', marginTop: '20px', background: 'linear-gradient(135deg, #10b981, #059669)'}}
                  onClick={() => {
        ...
        {showAddForm && (
          <div style={styles.glassCard}>
            <h2 style={{...styles.title, fontSize: '2rem', marginBottom: '25px'}}>
                ...
                <div style={styles.weightContainer}>
                  ...
                  >
                    {weightUnits.map(unit => (
                      <option key={unit} value={unit} style={{background: '#1f2937', color: 'white'}}>
                        {unit}
                      </option>
                    ))}
                  </select>
        ...
        <div style={styles.glassCard}>
              ...
              <tbody>
                {filteredProducts.map((product) => (
                  ...
                  >
                    ...
                    <td style={{...styles.tableCell, fontWeight: 'bold', fontSize: '1.1rem'}}>
                      ₹{product.pricePerUnit}
                    </td>
                    <td style={{...styles.tableCell, fontWeight: 'bold', fontSize: '1.1rem'}}>
                      ₹{calculatePrice(product.pricePerUnit, product.weight, product.weightUnit).toFixed(2)}
                    </td>
                    <td style={styles.tableCell}>
                      <button
          ...
          {filteredProducts.length === 0 && (
            <div style={styles.emptyState}>
              ...
              <h3 style={{fontSize: '1.5rem', margin: '20px 0 10px'}}>No products found</h3>
